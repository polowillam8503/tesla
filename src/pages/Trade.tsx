...
export const Trade: React.FC<{ defaultCoinId?: string }> = ({ defaultCoinId }) => {
  ...
  const onBookClick = (p: number) => { setInputPrice(p.toFixed(2)); setPriceType('LIMIT'); };
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
        try {
            ...
            let c=Number(selectedCoin.current_price || 100); const now=Math.floor(Date.now()/1000); const arr=[];
            for(let i=0;i<100;i++){const t=now-i*900;const v=c*0.01;const cl=c+(Math.random()-0.5)*v;const o=c;const h=Math.max(o,cl)+v*0.2;const l=Math.min(o,cl)-v*0.2;arr.unshift({time:t as any,open:o,high:h,low:l,close:cl});c=cl;}
            series.setData(arr);
        }
    };
    fetchHistory();
    const ro = new ResizeObserver(() => { if(chartContainerRef.current && chartInstance.current) { chartInstance.current.applyOptions({ width: chartContainerRef.current.clientWidth, height: chartContainerRef.current.clientHeight }); }});
  ...
  const renderOrderBook = () => (<div className="flex-1 flex flex-col h-full bg-[#181a20]"><div className="px-4 py-2 grid grid-cols-3 text-[10px] text-[#848e9c] font-bold uppercase border-b border-white/5"><span>{t('price')}</span><span className="text-right">{t('amount')}</span><span className="text-right">{t('total')}</span></div><div className="flex-1 overflow-hidden flex flex-col relative"><div className="flex-1 flex flex-col-reverse justify-end overflow-hidden pb-1">{[...Array(15)].map((_, i) => { const askPrice = (Number(selectedCoin.current_price || 0)) * (1 + (0.0005 * (i+1))); const amount = Math.random() * 2; return (<div key={i} className="grid grid-cols-3 px-4 py-[2px] text-xs relative group cursor-pointer hover:bg-[#1e2329]" onClick={() => onBookClick(askPrice)}><div className="absolute top-0 right-0 h-full bg-[#f6465d]/10 pointer-events-none" style={{ width: `${Math.random()*100}%` }}></div><span className="text-[#f6465d] relative z-10 font-mono group-hover:font-bold">{Number(askPrice || 0).toFixed(2)}</span><span className="text-[#eaecef] text-right relative z-10 font-mono opacity-90">{Number(amount || 0).toFixed(3)}</span><span className="text-[#eaecef] text-right relative z-10 font-mono opacity-60">{Number((askPrice * amount) || 0).toFixed(1)}</span></div>); })}</div><div className="py-2 px-4 border-y border-white/5 bg-[#1e2329] flex items-center justify-between sticky top-0 z-20"><div className="flex items-center gap-2"><span className={`text-lg font-bold font-mono ${(Number(selectedCoin.price_change_percentage_24h || 0)) >= 0 ? 'text-[#0ecb81]' : 'text-[#f6465d]'}`}>{Number(selectedCoin.current_price || 0).toFixed(2)}</span>{(Number(selectedCoin.price_change_percentage_24h || 0)) >= 0 ? <ArrowUp size={14} className="text-[#0ecb81]"/> : <ArrowDown size={14} className="text-[#f6465d]"/>}</div></div><div className="flex-1 overflow-hidden pt-1">{[...Array(15)].map((_, i) => { const bidPrice = (Number(selectedCoin.current_price || 0)) * (1 - (0.0005 * (i + 1))); const amount = Math.random() * 2; return (<div key={i} className="grid grid-cols-3 px-4 py-[2px] text-xs relative group cursor-pointer hover:bg-[#1e2329]" onClick={() => onBookClick(bidPrice)}><div className="absolute top-0 right-0 h-full bg-[#0ecb81]/10 pointer-events-none" style={{ width: `${Math.random()*100}%` }}></div><span className="text-[#0ecb81] relative z-10 font-mono group-hover:font-bold">{Number(bidPrice || 0).toFixed(2)}</span><span className="text-[#eaecef] text-right relative z-10 font-mono opacity-90">{Number(amount || 0).toFixed(3)}</span><span className="text-[#eaecef] text-right relative z-10 font-mono opacity-60">{Number((bidPrice * amount) || 0).toFixed(1)}</span></div>); })}</div></div></div>);
