...
export const Admin: React.FC = () => {
  ...
  const emptyToken: CustomTokenConfig = {
      ...
      contractAddress: '', priceChangePercent: 0
  };
  ...
  const [tokenEdits, setTokenEdits] = useState<Record<string, { price: string, change: string }>>({});
  ...
  const handleTokenEditChange = (symbol: string, field: 'price' | 'change', value: string) => { setTokenEdits(prev => ({ ...prev, [symbol]: { ...prev[symbol], [field]: value } })); };
  const saveTokenEdit = (token: any) => { 
      ...
      updateCustomToken(token.symbol, { price, priceChangePercent: change }); 
  };
                        ...
                        {miningRigs.map((rig) => (
                            <div key={rig.id} className="bg-[#0b0e11] border border-white/10 rounded-xl p-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        <label className="text-xs text-[#848e9c] uppercase block mb-1">Daily Output (TSLA)</label>
                                        <input 
                                            type="number" 
